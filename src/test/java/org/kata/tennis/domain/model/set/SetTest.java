...
public class SetTest {
	...
	public void firstPlayerShouldWinSet() throws UnknownPlayerException {
		...
		// THEN
		Assert.assertEquals(FIRST_PLAYER, this.set.getWinner().orElse(new Player("nobody")));
	}
	...
	public void playerShouldWinSetIfHeWinTheTieBreak() throws UnknownPlayerException {
		...
		// THEN
		Player setWinner = this.set.getWinner().orElse(new Player("nobody"));
		Player tieBreakWinner = tieBreak.getWinner().orElse(new Player("nobody"));
