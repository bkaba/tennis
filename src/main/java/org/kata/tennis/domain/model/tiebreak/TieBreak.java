...
public class TieBreak {
	...
	public void scores(Player player) throws UnknownPlayerException {
		if (Objects.isNull(winner)) {
			final List<Integer> scores = Optional.ofNullable(this.tieBreakScores.get(player))
	...
	private void incrementScore(Player player, List<Integer> scores) {
		...
		this.tieBreakScores.entrySet().stream().filter(o -> !o.getKey().equals(player))
	...
	private boolean isTieBreakWinnable(Player player) {
		...
		final int playerTwoScore = getLastScore(
				this.tieBreakScores.entrySet().stream().filter(o -> !o.getKey().equals(player)).map(Map.Entry::getValue)
